...
export default function Interview() {
    ...
    const handleEndCall = useCallback(async () => {
        ...
        try {
            ...
            const voices = {
                interviewer: conversationTranscript.hr || "No speech recorded",
                candidate: conversationTranscript.user || "No speech recorded"
            }

            const formData = { jobId: interviewDetails?.jobId?._id, status: "completed", conversationTranscript: voices };
            if(user?.role === "User") {
                formData.applicantId = user?._id;
            }

            const response = await axios.put(`http://localhost:3000/api/interview/completeInterview/${id}`, formData, { headers: { Authorization: token } });
            ...
            // Still show transcript summary even if API call fails
            setShowTranscriptSummary(true);
        }
        ...
        peer.close();
    }, [myStream, remoteStream, listening, resetTranscript, saveSpeechToConversation, id, conversationTranscript]);
