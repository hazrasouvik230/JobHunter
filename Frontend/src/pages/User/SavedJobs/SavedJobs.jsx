...
//       // Update local state - remove the job object from savedJobs array
...
//                   <p className='text-xs'>Posted on: {formatDistanceToNow(new Date(job.createdAt))} ago</p>
//                   <p className='text-xs'>{new Date(job.deadline) > Date.now() ? `Deadline in ${formatDistanceToNow(new Date(job.deadline))}` : `Deadline was ${formatDistanceToNow(new Date(job.deadline))} ago`}</p>
//                 </div>

//                 {/* Always show filled bookmark in SavedJobs page since all jobs here are saved */}
//                 <FaBookmark 
//                   className='absolute right-4 top-4 text-gray-800 cursor-pointer hover:scale-110' 
//                   onClick={() => handleUnsave(job._id)} 
//                 />

//                 <button 
//                   className={`absolute right-4 bottom-4 py-2 px-8 font-semibold rounded cursor-pointer ${user.appliedJobs.includes(job._id) ? "bg-green-200 text-green-800" : " bg-orange-300/50 text-orange-800 hover:scale-110"}`} 
//                   onClick={() => navigate(`/user/apply-job/${job._id}`)}
//                 >
//                   {user.appliedJobs.includes(job._id) ? "Applied" : "Show details"}
//                 </button>
//               </div>
//             );
//           })}
  ...
  const handleUnsave = async (jobId) => {
    try {
      ...
      if (savedJobs.length === 1 && page > 1) {
        ...
        // If it's the last job on the first page, just fetch again
        fetchSavedJobs();
      }
